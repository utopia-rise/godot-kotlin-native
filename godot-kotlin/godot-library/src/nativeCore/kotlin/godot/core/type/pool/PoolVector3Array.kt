...
class PoolVector3Array : NativeCoreType<godot_pool_vector3_array_layout>, Iterable<Vector3> {
    override var _handle = cValue<godot_pool_vector3_array_layout>{}
    ...
    internal constructor(native: CValue<godot_pool_vector3_array_layout>) {
        memScoped {
            this@PoolVector3Array._setRawMemory(native.ptr)
        }
    }
    ...
    fun append(vector: Vector3) {
        callNative {
            nullSafe(Godot.gdnative.godot_pool_vector3_array_append)(it, vector._getRawMemory(this).reinterpret())
        }
    }
    ...
    fun insert(idx: Int, data: Vector3) {
        callNative {
            nullSafe(Godot.gdnative.godot_pool_vector3_array_insert)(it, idx, data._getRawMemory(this).reinterpret())
        }
    }
    ...
    fun pushBack(data: Vector3) {
        callNative {
            nullSafe(Godot.gdnative.godot_pool_vector3_array_push_back)(it, data._getRawMemory(this).reinterpret())
        }
    }
    ...
    operator fun set(idx: Int, data: Vector3) {
        callNative {
            nullSafe(Godot.gdnative.godot_pool_vector3_array_set)(it, idx, data._getRawMemory(this).reinterpret())
        }
    }
